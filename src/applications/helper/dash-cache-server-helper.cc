...
//
#include "dash-cache-server-helper.h"
#include "ns3/http-server.h"
#include "ns3/http-client.h"
#include "ns3/dash-fake-cache-server.h"
...
{
  m_factory.SetTypeId (DASHFakeCacheServer::GetTypeId ());
...
void DASHCacheServerHelper::SetAttribute (std::string name, const AttributeValue &value)
{
  m_factory.Set (name, value);
}

ApplicationContainer DASHCacheServerHelper::Install (Ptr<Node> node) const
{
  return ApplicationContainer (InstallPriv (node));
}

ApplicationContainer DASHCacheServerHelper::Install (std::string nodeName) const
{
  Ptr<Node> node = Names::Find<Node> (nodeName);
  return ApplicationContainer (InstallPriv (node));
}

ApplicationContainer DASHCacheServerHelper::Install (NodeContainer c) const
{
...
Ptr<Application> DASHCacheServerHelper::InstallPriv (Ptr<Node> node) const
{
  Ptr<Application> app = m_factory.Create<DASHFakeCacheServer> ();
