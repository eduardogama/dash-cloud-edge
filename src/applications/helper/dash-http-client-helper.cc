...
#include "ns3/http-client-dash.h"
...
void DASHHttpClientHelper::SetAttribute (
  std::string name,
...
ApplicationContainer DASHHttpClientHelper::Install (Ptr<Node> node) const
{
  return ApplicationContainer (InstallPriv (node));
}

ApplicationContainer DASHHttpClientHelper::Install (std::string nodeName) const
{
  Ptr<Node> node = Names::Find<Node> (nodeName);
  return ApplicationContainer (InstallPriv (node));
}

ApplicationContainer DASHHttpClientHelper::Install (NodeContainer c) const
{
...
Ptr<Application> DASHHttpClientHelper::InstallPriv (Ptr<Node> node) const
{
